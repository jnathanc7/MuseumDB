...
module.exports = (req, res) => {
  ...
  if (method === "POST" && parsedUrl.pathname === "/purchase") {
    ...
    req.on("end", () => {
      try {
        // Now expecting customer_ID in the request body instead of extracting it from req.user.
        const { payment_Method, tickets, customer_ID } = JSON.parse(body);
  
        if (!customer_ID) {
          res.writeHead(400, { "Content-Type": "application/json" });
          return res.end(JSON.stringify({ error: "customer_ID is required" }));
        }
